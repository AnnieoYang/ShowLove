...
def closeWindow():
    messagebox.showinfo(message="再考虑考虑吧")
...
def Love():
#顶级窗口
    love = Toplevel(window)#创建一个顶级弹窗
    love.geometry("300x100+520+260")
    love.title("")
    label = Label(love,text = "我就知道you会同意的^—^",font = ("微软雅黑",18)) 
    label.pack()#自动调节组件的合适位置
...
def noLove():
    ...
    if a == 1:
        ...
        label.pack()
        btn = Button(no_love, text = "我再考虑一下",width = 10, height = 2, command = no_love.destroy)
        btn.pack()
...
#标签控制
label = Label(window, text = "小姐姐\n我观察你很久了\n作我女朋友好不好", font = ("微软雅黑",15),justify = LEFT, padx = 10)
...
#显示图片
photo = PhotoImage(file = "bloodcat.gif")
